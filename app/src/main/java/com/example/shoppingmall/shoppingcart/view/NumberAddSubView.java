...
public class NumberAddSubView extends LinearLayout implements View.OnClickListener {
    ...
    public NumberAddSubView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            //取出属性
            @SuppressLint("RestrictedApi") TintTypedArray tintTypedArray = TintTypedArray.obtainStyledAttributes(context, attrs, R.styleable.NumberAddSubView);
            @SuppressLint("RestrictedApi") int value = tintTypedArray.getInt(R.styleable.NumberAddSubView_value, 0);
            if (value > 0) {
                setValue(value);
            }
            @SuppressLint("RestrictedApi") int minValue = tintTypedArray.getInt(R.styleable.NumberAddSubView_minValue, 0);
            if (value > 0) {
                setMinValue(minValue);
            }
            @SuppressLint("RestrictedApi") int maxValue = tintTypedArray.getInt(R.styleable.NumberAddSubView_maxValue, 0);
            if (value > 0) {
                setMaxValue(maxValue);
            }
            @SuppressLint("RestrictedApi") Drawable addDrawable = tintTypedArray.getDrawable(R.styleable.NumberAddSubView_numberAddBackground);
            if (addDrawable != null) {
